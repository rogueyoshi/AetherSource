...
void CDirectXWrapper::ClearScreen(float red, float green, float blue, float alpha)
{
	// Clear the views.
	m_d3dContext->ClearRenderTargetView(m_renderTargetView.Get(), XMVECTORF32{ red, green, blue, alpha });
...
HBITMAP CDirectXWrapper::Capture()
{
	...
	m_iBuffer.resize(textureDesc.Width * textureDesc.Height * 4);
	auto destination = m_iBuffer.data();
